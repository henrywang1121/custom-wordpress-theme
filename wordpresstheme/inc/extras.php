...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	...
	function custom_wp_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			...
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'custom' ), max( $paged, $page ) );
		}
		return $title;
	}
 ...
 * @return void
 */
function custom_setup_author() {
	global $wp_query;
	if ( $wp_query->is_author() && isset( $wp_query->post ) ) {
		$GLOBALS['authordata'] = get_userdata( $wp_query->post->post_author );
	}
}
add_action( 'wp', 'custom_setup_author' );

/**
 * Adds Skype URI support for social media icons.
 *
